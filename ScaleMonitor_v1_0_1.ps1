# ===========================================================
# ScaleMonitor_v1_0_1.ps1 by Shawn.lee@sgs.com
# Mettler Toledo & OHAUS Scale Monitor - Scale to Excel via COM API
# Version: 1.0.1
# Description: 加入防連發 (Debounce) 機制，解決 OHAUS 天平等廠牌單次傳輸多筆數據的問題。
# ===========================================================
...
function Show-SetupWizard {
    ...
    $v = Read-Host "  防連發過濾時間 (秒) [$($defaults.debounceSeconds)]"
    $cfg.debounceSeconds = if ($v) { [int]$v } else { $defaults.debounceSeconds }
...
if ($runSetup) {
    $currentDefaults = if ($cfg) {
        @{
            ...
            debounceSeconds    = if ($null -ne $cfg.debounceSeconds) { $cfg.debounceSeconds } else { 1 }
        }
    }
    else { $defaults }
    ...
    # 針對舊版 config.json 沒有 debounceSeconds 的情況設定預設值
    if ($null -eq $cfg.debounceSeconds) {
        $cfg | Add-Member -MemberType NoteProperty -Name "debounceSeconds" -Value 1
    }
}
...
Write-Host "  Version: 1.0.1" -ForegroundColor DarkCyan
Write-Host "------------------------------------------" -ForegroundColor Cyan
...
Write-Host "  防連發過濾: $($cfg.debounceSeconds) 秒" -ForegroundColor Cyan
Write-Host ""
    ...
    while ($true) {
        ...
        try {
            ...
            if ($data -match '([-+]?\d+\.\d+)\s*(g|mg|kg|ct|oz|lb)\b') {
                ...
                # --- 防連發 (Debounce) 機制 ---
                if (((Get-Date) - $lastInputTime).TotalSeconds -lt $cfg.debounceSeconds) {
                    Write-Host "[$(Get-Date -Format 'HH:mm:ss')]   Ignored (Debouncing): $($data.Trim())" -ForegroundColor DarkGray
                    continue
                }
